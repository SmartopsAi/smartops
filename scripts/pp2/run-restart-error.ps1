...
Write-Host "`n== Enable error burst on ALL simulator pods =="
kubectl get pods -n $ns -l app=smartops-erp-simulator -o name | ForEach-Object {
    kubectl exec -n $ns $_ -- python -c "import urllib.request; print(urllib.request.urlopen('http://127.0.0.1:8000/chaos/error-burst/enable', data=b'', timeout=5).read().decode()); print(urllib.request.urlopen('http://127.0.0.1:8000/chaos/modes', timeout=5).read().decode())"
}

Write-Host "`n== Recreate loadgen pod manifest =="
@'
apiVersion: v1
kind: Pod
metadata:
        ...
        - |
          ...
          while [ $i -le 20 ]; do
            curl -s -o /dev/null -w "%{http_code}\n" \
              ...
              -d '{"duration_seconds":0.2,"target":"error"}'
            i=$((i+1))
            sleep 1
          done
...
Write-Host "`n== Detector status after error traffic =="
kubectl logs -n $ns deploy/smartops-agent-detect-sim --tail=80

Write-Host "`n== Latest anomaly lines =="
kubectl logs -n $ns deploy/smartops-orchestrator --tail=600 | Select-String "SIGNAL_RAW | kind=anomaly"

Write-Host "`n== Latest restart summary lines =="
kubectl logs -n $ns deploy/smartops-orchestrator --tail=600 | Select-String "restart_on_anomaly_error|CLOSED_LOOP_SUMMARY"

Write-Host "`n== Latest policy audit =="
kubectl run -n $ns curlpod --image=curlimages/curl:8.5.0 --restart=Never -i --rm -- `
  curl -sS "http://smartops-policy-engine:5051/v1/policy/audit/latest?n=15"