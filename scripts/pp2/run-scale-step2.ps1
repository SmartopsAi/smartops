...
Write-Host "== Pre-check: deployment state before step 2 =="
...
Write-Host "`n== Keep CPU spike enabled on ALL simulator pods =="
...
Write-Host "`n== Generate traffic again for second escalation =="
...
Write-Host "`n== Detector status after second CPU spike traffic =="
...
kubectl logs -n $ns deploy/smartops-orchestrator --tail=600 | Select-String "CLOSED_LOOP_SUMMARY"
